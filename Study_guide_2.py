"""
In this we will use Langgraph to provide orchestration for the same functionality
...
# Shared state passed between the nodes
...
# Node 2: wrtie notes from outline
...
# Node 3: wrtite recview questions from the notes
def quiz_writer(state: StudyState) -> dict:
    return {
        "quiz": run_node(
            "quiz_writer",
            "Write 5 short review questions based on the notes.",
            f"Topic: {state['topic']}\n\nNotes:\n{state['notes']}",
        )
    }
